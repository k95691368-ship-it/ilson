...
import { resolveOverrideActor, requireOverridePermission } from '../../../_lib/override.js'
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const operations = { products, events, experiments, runs, decisions }
...
export async function onRequestPost({ env, data: requestData, request, params }) {
  ...
  try {
    ...
    if (!env.DEMO_WORKSPACE) {
      const actor = await resolveOverrideActor({ ...env, OVERRIDE_DEMO_MODE: 'false' }, request, body)
    ...
    const product = await env.DB.prepare('SELECT id FROM override_product WHERE id=?').bind(body.productId).first()
