...
export async function onRequestGet({ env, data: requestData }) {
  ...
  try {
    ...
    const evidence = await loadOutcomeEvidenceMany(env.DB, unresolved.results.map(row => row.id))
    for (const row of unresolved.results) {
      const current = evidence.get(row.id)
      if (!current || current.outcome.status === '산정불가') continue
      for (const challenge of current.challenges.filter(item => !item.resolved_at)) {
        openChallenges.push({ rule_code: challenge.code, title: challenge.title, body: challenge.body, ticket_no: row.ticket_no })
      }
    }
