...
// 여덟 건이 들어온다. 그리고 거기서 끝난다.
//
// 처음 오신 분 입장에서 보면 이렇다. 여덟 단계 목차가 있고, 접수함에
// 신청서가 여덟 건 있다. 그런데 나머지 일곱 단계는 전부 비어 있다.
...
const STEPS = [
  ...
  },
  {
    ...
    label: '무엇까지 되면 성공인지 정하기',
    what: '수용한 건에 합격 기준을 적습니다. 금액 오차 0원 같은 것입니다.',
    need: '만든 뒤에 정하면 결과에 맞춰 기준이 움직입니다. 그래서 여기서 먼저 받습니다.',
    shows: '부서가 그 기준을 항목마다 보고 맞다/아니다를 누를 수 있게 됩니다.',
    where: '/agreement',
  },
  {
    key: 'build',
    ...
    label: '만들기 전에 사람이 세 번 해 보기',
    what: '그 일을 사람이 직접 하는 데 몇 분 걸리는지를 재서 봉인합니다.',
    need: '이 값이 없으면 나중에 "얼마나 줄었나"를 셀 바닥이 없습니다. 만든 뒤에 재면 그 숫자는 못 믿습니다.',
    shows: '그다음 파일을 올리면 규칙이 합치고, 못 읽은 줄은 버리지 않고 따로 뺍니다.',
    where: '/build',
  },
  {
    key: 'beta',
