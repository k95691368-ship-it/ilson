...
export default function FieldFeedbackView({ mode, role, products, onCapture }) {
  const { data, error, loading, reload } = useApi(`/feedback?role=${encodeURIComponent(role)}`)
  ...
  async function save(payload, form) {
    ...
    try {
      ...
      form?.reset()
      if (payload.action === 'create_sample') setSelected(result.id)
  ...
  return <section className="ol-page field-workspace" data-clarity-mask="true">
    <header className="ol-page-intro"><div><span className="ol-eyebrow">OverrideLoop / {mode === 'feedback' ? '피드백' : '품질'}</span>
      <h1>{mode === 'feedback' ? '피드백, 그다음까지.' : '승인 뒤에도 확인합니다.'}</h1>
      <p>{mode === 'feedback' ? '담당자의 답변을 확인하고, 적용된 개선이 실제로 도움이 됐는지 남깁니다.' : '기록된 승인 사건을 점검하고, 도구를 사용하지 않는 이유를 듣습니다.'}</p></div>
    ...
    {data && mode === 'feedback' && <>
      <div className="field-caption"><span>읽지 않은 안내 <strong>{data.unread}건</strong></span><span>{data.manager ? '담당자 권한 · 최근 피드백 최대 100건' : '내가 남긴 최근 피드백 최대 100건'}</span></div>
      {!data.cases.length && <div className="ol-panel field-empty"><h2>새 판단 기록부터 연결됩니다.</h2><p>수정·거절·이관 사건을 남기면 이곳에서 처리 안내를 받습니다. 작성자가 확인되지 않는 과거 시연 기록은 내 피드백으로 표시하지 않습니다.</p><button className="ol-primary" onClick={onCapture}>판단 기록하기</button></div>}
    ...
    {data && mode === 'quality' && <>
      {data.reviewer ? <>
        ...
        {batch && <section className="ol-panel"><label className="field-batch-picker">점검 묶음<select value={batch.id} onChange={e => setSelected(e.target.value)}>{data.batches.map(b => <option key={b.id} value={b.id}>{b.product_name} · {b.start_at} ~ {b.end_at} · {b.id}</option>)}</select></label>
          <div className="field-metrics"><div><span>추출 당시 대상</span><strong>{batch.eligible_count}건</strong></div><div><span>추출 / 요청</span><strong>{batch.sample_size} / {batch.requested_size}건</strong></div><div><span>점검 완료</span><strong>{summary.reviewed}건</strong></div><div><span>문제 / 근거 부족</span><strong>{summary.issues} / {summary.insufficient}건</strong></div></div>
          ...
          {samples.map(item => <article className="field-sample" key={item.id}><h3>사건 {item.event_id}</h3><dl className="field-snapshot"><div><dt>AI 답변</dt><dd>{item.snapshot.ai_decision}</dd></div><div><dt>직원 판단</dt><dd>{item.snapshot.human_decision}</dd></div><div><dt>모델 / 프롬프트</dt><dd>{item.snapshot.model_version || '미기록'} / {item.snapshot.prompt_version || '미기록'}</dd></div><div><dt>정책 근거</dt><dd>{safeJson(item.snapshot.policy_refs_json, []).join(' · ') || '미기록'}</dd></div><div><dt>원 사건 시각</dt><dd>{item.snapshot.occurred_at || '미기록'} (UTC)</dd></div></dl>
            {item.verdict ? <div className="field-status"><strong>{QUALITY_VERDICTS[item.verdict]}</strong><p>{item.reason}</p><p>근거: {item.evidence_refs || '근거 부족으로 미기록'} · {item.reviewed_by}</p></div> : <form className="field-form" onSubmit={e => submit(e, 'review_sample', { itemId: item.id })}>
